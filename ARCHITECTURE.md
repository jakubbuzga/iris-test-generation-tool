# Iris Test Generation Tool Architecture
...
- **Specification Management**: Upload and manage OpenAPI specifications.  
- **AI Test Generation**: Generate complete API tests using an LLM, including Postman test scripts (assertions).  
- **Interactive Feedback Loop**: Provide feedback (good/bad) on generated tests to save valid ones and refine invalid ones.  
- **Unified Collection Management**: Save validated tests into manageable Postman collections and import existing collections to manage all tests in one place.  
- **Test Execution**: Run Postman collections directly from the application.  
- **Reporting and History**: View detailed, historical results for every collection run.  
- **Filtering**: Easily filter the test run history by the associated collection and its final status.  

---

## 2. High-Level Architecture

The system is designed as a distributed, service-oriented architecture to ensure **separation of concerns**, **scalability**, and **maintainability**. This model isolates responsibilities, allowing each component to be developed, deployed, and scaled independently.

### Components

- **Frontend**:  
  A client-side web application providing the user interface (React.js). It is the single point of interaction for the user.

- **Backend**:  
  The central API server (Node/Express) that handles business logic, user management, data persistence, and orchestrates communication between all other components.

- **Database**:  
  A PostgreSQL database serving as the single source of truth for all persistent data, including user info, specifications, collections, and run results.

- **AI Agent**:  
  A dedicated microservice (built with Langchain) that encapsulates the logic for interacting with the LLM. It parses specifications, engineers prompts for generation and refinement, and formats the LLM's output.

- **LLM Model**:  
  A pre-trained Large Language Model (e.g., from OpenAI, Anthropic) that provides the core generative capabilities.

- **Test Runner Service**:  
  An isolated microservice responsible for executing Postman collections via Newman and reporting detailed results back to the Backend.
